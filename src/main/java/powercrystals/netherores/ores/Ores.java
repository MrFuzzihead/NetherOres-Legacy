...
public enum Ores {
    ...
    public void load() {
        NetherOresCore.getOreBlock(this._blockIndex)
            .setHarvestLevel("pickaxe", this._miningLevel, this._metadata);
        ...
        if (!this._registeredSmelting) {
            ...
            if (NetherOresCore.enableStandardFurnaceRecipes.getBoolean(true)) {
                ...
                FurnaceRecipes.smelting()
                    .func_151394_a(this.getItemStack(1), var2, 1.0F);
            }
    ...
    private void registerAEGrinder(ItemStack var1) {
        ...
        IGrinderRegistry var3 = AEApi.instance()
            .registries()
            .grinder();
    ...
    public void loadConfig(Configuration var1) {
        ...
        this._oreGenMaxY = var1.get(var2, "MaxY", this._oreGenMaxY)
            .setRequiresMcRestart(true)
            .getInt();
        this._oreGenMinY = var1.get(var2, "MinY", this._oreGenMinY)
            .setRequiresMcRestart(true)
            .getInt();
        if (this._oreGenMinY >= this._oreGenMaxY) {
            this._oreGenMinY = this._oreGenMaxY - 1;
            var1.get(var2, "MinY", this._oreGenMinY)
                .set(this._oreGenMinY);
        }

        this._oreGenGroupsPerChunk = var1.get(var2, "GroupsPerChunk", this._oreGenGroupsPerChunk)
            .setRequiresMcRestart(true)
            .getInt();
        this._oreGenBlocksPerGroup = var1.get(var2, "BlocksPerGroup", this._oreGenBlocksPerGroup)
            .setRequiresMcRestart(true)
            .getInt();
        this._oreGenDisable = var1
            .get(var2, "Disable", false, "Disables generation of " + this.name() + " (overrides global ForceOreSpawn)")
            .setRequiresMcRestart(true)
            .getBoolean(false);
        this._oreGenForced = var1.get(var2, "Force", false, "Force " + this.name() + " to generate (overrides Disable)")
            .setRequiresMcRestart(true)
            .getBoolean(false);
        this._miningLevel = var1
            .get(var2, "MiningLevel", this._miningLevel, "The pickaxe level required to mine " + this.name())
            .setRequiresMcRestart(true)
            .getInt();
        this._retroGenEnabled = var1
            .get(var2, "Retrogen", true, "Retroactively generate " + this.name() + " if enabled in CoFHCore")
            .setRequiresMcRestart(true)
            .getBoolean(true);
        var2 = "Processing.Ores." + this.name();
        this._smeltCount = var1.get(var2, "SmeltedCount", this._smeltCount, "Output from smelting " + this.name())
            .setRequiresMcRestart(true)
            .getInt();
        this._primary = var1
            .get(
                ...
                "PrimaryOrePrefix",
                this._primary,
                "Output from smelting " + this.name()
                    + " if ore"
                    + this.name()
            ...
            .setRequiresMcRestart(true)
            .getString();
        this._pulvCount = var1.get(var2, "PulverizedCount", this._pulvCount, "Output from grinding " + this.name())
            .setRequiresMcRestart(true)
            .getInt();
        this._secondary = var1
            .get(
                var2,
                "AlternateOrePrefix",
                this._secondary,
                "Output from grinding " + this
                    .name() + " if dust" + this.name() + " is not found (i.e., " + this._secondary + this.name() + ")")
            .setRequiresMcRestart(true)
            .getString();
    }
    ...
    public void postConfig(Configuration var1) {
        ...
        if (!var1
            .get(
                ...
                "Disable",
                this._oreGenDisable,
                "Disables generation of " + this.name() + " (overrides global ForceOreSpawn)")
            .wasRead()) {
            var1.get(
                var2,
                "Disable",
